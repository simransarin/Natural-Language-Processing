...
def read_input(files, test_data):
    ...
    for file in files:
        ...
        evaluate = [
            lexical_unit.lower() for lexical_unit in evaluate if lexical_unit not in stopwords]
        evaluate = [
            lexical_unit for lexical_unit in evaluate if lexical_unit.isalpha()]
        test_data.append([evaluate, file])
    naive_bayes_classify(test_data)

def write_output(predict, output_file, i):
    if predict == 'd_neg':
...
def naive_bayes_classify(test_data):
    ...
    for i in range(len(test_data)):
        ...
        write_output(predict, output_file, i)
    output_file.close()

test_data = []
files = glob.glob(os.path.join(sys.argv[1], '*/*/*/*.txt'))
read_input(files, test_data)
